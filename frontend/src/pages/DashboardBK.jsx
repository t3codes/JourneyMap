...
const Dashboard = () => {
  ...
  const handleCountryClick = (country) => {
    const countryData = getUserCountryData(country.nome_comum);
    if (countryData) {
      showAlert(
        'info', 
        country.nome_comum,
        `Capital: ${country.capital} | Região: ${country.regiao} | População: ${(country.populacao / 1000000).toFixed(1)}M`
      );
    } else {
      showAlert(
        'info', 
        country.nome_comum,
        `Capital: ${country.capital} | Região: ${country.regiao} | População: ${(country.populacao / 1000000).toFixed(1)}M | Este país não está na sua lista.`
      );
    }
  };
